...
#define txPin 2
#define rxPin 3
SoftwareSerial Serial1(rxPin,txPin, SERIAL_8E1);

#define Usart_stm32 Serial
#define Usart_arduino Serial1
...
//  Usart_arduino.begin(57600);
  ...
  if(!SD.begin(chipSelect)) Usart_arduino.println(F("Card failed - ERROR"));
  else Usart_arduino.println(F("Card initialized - OK"));
...
void loop() 
{
  if(Usart_arduino.available()) 
  {
    uint8_t res = Usart_arduino.read();
...
void entr_bootloader()
{
  ...
  if(ack_byte() == 0) Usart_arduino.println(F("Bootloader - OK"));  // Log
  else Usart_arduino.println(F("Bootloader - ERROR"));  // Log  
}
...
void write_memory()
{
  if(erase_memory() == 0)
  {
     ...
     if(df) 
     {
       ...
       Usart_arduino.print(F("Size file "));
       Usart_arduino.println(size_file);
       ...
       {      
         if(send_cmd(cmd_array) == 0)
         {
           ...
           {       
             ...
             //Usart_arduino.print(F("seek_len "));
             //Usart_arduino.println(seek_len);
             //Usart_arduino.println(df.position());
             ...
             if(ab != 0) 
             {
               Usart_arduino.println(F("Block not Write Memory - ERROR"));
               break;
             }
             
             if(size_file == df.position())
             {
               Usart_arduino.println(F("End Write Memory2 - OK"));
               boot_off_and_reset();
               break;
             }                 
           }
           ...
           {
             Usart_arduino.println(F("Address Write Memory - ERROR")); // Log
             break;
           }
         }
         else 
         {
           Usart_arduino.println(F("Cmd cod Write Memory - ERROR")); // Log
           break;
         }             
       } // end while
     ...
     else Usart_arduino.println(F("Not file - ERROR"));          
  } 
  else Usart_arduino.println(F("Not erase Write Memory - ERROR"));    
}
...
uint8_t erase_memory()
{
  ...
  if(send_cmd(cmd_array) == 0) 
  {
    ...
    if(send_cmd(cmd_array) == 0)
    {
      Usart_arduino.println(F("Erase Memory - OK")); // Log
      return 0;
    }
    else Usart_arduino.println(F("Cmd cod Erase Memory - ERROR")); // Log             
  }
  else Usart_arduino.println(F("Cmd start Erase Memory - ERROR")); // Log
...
void boot_off_and_reset()
{
  ...
  delay(500);
  on_reset(); 
  Usart_arduino.println(F("Boot off and reset")); // Log
}
