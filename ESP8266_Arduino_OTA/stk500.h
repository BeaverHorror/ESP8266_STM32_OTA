...
#define STK_SIGN_ON_MESSAGE "AVR STK"   // Sign on string for Cmnd_STK_GET_SIGN_ON   (Строка входа в систему для Cmnd_STK_GET_SIGN_ON)
...
#define Resp_STK_OK                0x10   // ' '   // OK. Отправляется после выполнения допустимой команды.
#define Resp_STK_FAILED            0x11   // ' '   // Failed. Отправляется, если выполнение команды завершилось с ошибкой.
#define Resp_STK_UNKNOWN           0x12   // ' '   // Unknown. Отправляется в ответ на неизвестную команду, если эта неизвестная команда непосредственно сопровождается Sync_CRC_EOP.
#define Resp_STK_NODEVICE          0x13   // ' '   // Nodevice. Отправляется в ответ на Cmnd_STK_ENTER_PROGMODE, если предварительно не были установлены правильные параметры устройства командой Cmnd_STK_SET_DEVICE.
#define Resp_STK_INSYNC            0x14   // ' '   // Insync. Означает установку синхронизации. Отправляется после приема команды Sync_CRC_EOP.
#define Resp_STK_NOSYNC            0x15   // ' '   // Nosync. Означает, что пропала синхронизация. Отправляется, если не была принята Sync_CRC_EOP после команды.
